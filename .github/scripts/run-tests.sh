#!/bin/bash
# Runs all 10 self-test assemblies through a built runner, writing each
...
run() {
  ...
  # that all 10 were actually produced). The `|| true` below is only for
  ...
  # empty result files, every job green. These 10 assemblies always
...
run ExpectoTests Style/ExpectoTests
