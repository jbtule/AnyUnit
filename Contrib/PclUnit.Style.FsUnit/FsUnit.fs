...
module TopLevelOperators =
    ...
    let internal shouldHelper (asserter:PclUnit.IAssert) (f : 'a -> #Constraint) x (y : obj) =
        let c = f x
        let y =
            match y with
    ...
    [<System.Obsolete("Uses Global Assertion, so pclunit goes into global assertion mode,  use `this.should` instead")>]
    let should (f : 'a -> #Constraint) x (y : obj) =
        shouldHelper PclUnit.Run.Assert.GlobalStyle f x y
    ...
    let internal shouldFailHelper (asserter:PclUnit.IAssert) (f : unit -> unit) =
        TestDelegate(f) |> shouldHelper asserter throw typeof<AssertionException>
          
    [<System.Obsolete("Uses Global Assertion, so pclunit goes into global assertion mode, use `this.shouldFail` instead")>]
    let shouldFail (f : unit -> unit) =
        shouldFailHelper PclUnit.Run.Assert.GlobalStyle f
